...
//2. 반환값이 없는 함수 : 반환값이 없는 void 함수 선언
...
// 5. 함수를 변수에 저장
// String test = multiply; // 타입이 맞지 않아 오류가 남
...
void main(){
  ...
  greet("이순신", "안녕히가세요!"); // 3번 함수 호출 선택적 매개변수 전달.
  // greet(); // 3번 함수 호출 에러 name의 매개변수는 선택적 매개변수가 아니기 때문에 오류 발생.

  print(
    " 4 * 5 = ${multiply(4,5)}"); // 4번 익명함수 호출 (retrun 의 값이 화면에 출력되느 것을 볼 수 있다)
 
  executeFunction(multiply); // 4번에 정의되어 있는 함수를 매개변수로 받아 실행한다. 
  executeFunction(add); // 4번에 정의되어 있는 함수를 매개변수로 받아 실행한다. 

  var retrunedFunction = getFunction();
  print("10 + 20 = ${retrunedFunction(10,20)}"); // 출력 : 10 + 20 = 30
